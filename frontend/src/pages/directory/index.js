...
const Directory = () => {
    ...
    if (groups.length) {
      ...
      for (const group of groups) {
        if (group.fields.Label && group.fields.Label.length) {
          for (const label of group.fields.Label) {
  ...
  function toggleCategory(category) {
    ...
    if (selectedCategories.has(category)) {
      const selectedCategoriesCopy = new Map(selectedCategories)
  ...
  if (loading)
    ...
    const groupsFirstColumn = [...filteredGroups].splice(
      0,
      filteredGroups.length / 2 + 1
    )
    const groupsSecondColumn = [...filteredGroups].splice(
      filteredGroups.length / 2 + 1
    )
    showGroups = (
    ...
    <section className={classes.Directory}>
      ...
      <div className={classes.Description}>
        ...
        <p>
          ...
          getting your organization added to our directory?
          <br />
          <br />
      ...
      <div className={classes.Description}>
        ...
        <p>
          ...
          Queens. These groups are providing community services, keeping our
          infrastructure running, and aiding our neighbors in their times of
          need. Many of these organizations are run by volunteers and are always
          in need of a helping hand, which is why we’ve also compiled a list of{' '}
          <Link to="/roles">Open Volunteer Roles</Link> for those who want to
          get involved. This was created by a group of volunteers from{' '}
          <a href="http://astoria.digital/">Astoria Digital. </a>
        </p>
      </div>
    </section>
  )
